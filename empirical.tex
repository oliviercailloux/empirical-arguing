...
Let $F$ be a finite set of formulas is predicate logic with a deduction operator $⊢$. 

As we will see, negation of conjunctive formulas play a special role in the approach proposed by the authors. It will be convenient in what follows to be able to treat formulas such as $¬(a \land b)$ and $¬(b \land a)$ as the same mathematical object. To that effect, informally speaking, we define $\Delta$ as the set of formulas corresponding to $F$ except that in $\Delta$, negation of conjunctive formulas are considered to be equal when they only differ on the ordering of their conjuncts inside the negation. We also define $\land(\Phi) \in \Delta$, given $\Phi \subseteq \Delta$, as the conjunction of the formulas in $\Phi$, where the ordering of the formulas in $\Phi$ “does not matter”.
The following remark defines $\Delta$ and $\land(\Phi)$ formally, and may be skipped by readers who tolerate such informal parlance.
\begin{remark}
	To proceed formally, given $H \subseteq F$, let $\land(H) \subseteq F$ denote the set of conjunctions of the formulas in $H$ following whatever ordering of the formulas in $H$.
Define an equivalence relation $\equiv$ on $F$ as follows: two formulas are equivalent, $f \equiv g$, iff $f = g \lor [\exists H \subseteq F \suchthat f \in (¬{\land}(H)) \land g \in (¬{\land}(H))]$, thus, iff they are equal or they both negate the same conjunctive formula when ignoring the order of the sub-formulas in the conjunction. For example, given $a, b \in F$, $¬(a \land b) \equiv ¬(b \land a)$.
Define $\Delta = F / \equiv$ as the set of equivalence classes of $\equiv$, called (somewhat abusively) “unordered formulas”.
Please note that as a consequence, any unordered formula $\phi \in \Delta$ corresponds to a set of formulas $\set{f_1, f_2, …} \subseteq F$ which are equivalent as considered by $\equiv$; thus, $\phi$ can be viewed (and should be thought of) as a single formula “up to order”.
We extend $⊢$ on the domain of unordered formulas: given $\Phi \subseteq \Delta, \psi \in \Delta$, $\Phi ⊢ \psi$ iff $\forall f \in \psi: \cup(\Phi) ⊢ f$, or equivalently, iff $\exists f \in \psi \suchthat \cup(\Phi) ⊢ f$.
In what follows, we abuse vocabulary by calling an unordered formula a formula.
Note that $\land(\Phi) \in \Delta$.
In our examples of formulas, we display only one element of an equivalence class to represent the whole equivalence class.
\end{remark}

An argument is a pair $(\Phi, \alpha)$ with $\Phi \subseteq \Delta$ and $\alpha \in \Delta$ such that $\Phi ⊬ ⊥$ and such that $\Phi$ is a minimal subset of $\Delta$ satisfying $\Phi ⊢ \alpha$.
...
Example (given by the authors). Variables: $p$ for being a member of the Parliament; $q$ for keeping quiet about the person; $r$ for resigning from Parliament. Argument: $a = (\set{p ↔ ¬q, p}, ¬q)$. Undercut: $b = (\set{r, r → ¬p}, ¬p)$. Rebuttal: $(\set{r, r → ¬p, ¬p, p ↔ ¬q}, q)$.

As the authors prove, a defeater always admits a maximally conservative undercut. In the example, $b$ has an undercut $(\set{p, r → ¬p}, ¬r)$ and a maximally conservative undercut $(\set{p, r → ¬p}, ¬[r \land (r → ¬p)])$.
Given two formulas $\Phi, \Psi \in \Delta$, say that $\Psi$ is a canonical undercut for $\Phi$ iff $\Psi ⊢ ¬\land(\Phi)$.

Given an argument $(\Phi, \alpha)$, define the argumentation tree about $(\Phi, \alpha)$ as a tree of formulas whose root node is $\Phi$ and defined as follows. Given a node $\Psi$ in an argumentation tree, define $t[\Psi]$ as the set of its ancestors. Given any node $\Psi$, define its children as the subset of its canonical undercutters that satisfy $\Psi' \nsubseteq t[\Psi]$.
Note that an argumentation tree $t$ about $(\Phi, \alpha)$ is defined as a tree of formulas but also corresponds to a tree $t'$ of arguments having the same structure as $t$, where the root node of $t'$ is $(\Phi, \alpha)$ and each node $\Psi'$ in $t$ with child $\Psi$ in $t$ corresponds to an argument $(\Psi', ¬{\land}(\Psi))$ in $t'$.
