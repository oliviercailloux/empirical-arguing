...
Given any $\Phi, \Psi \subseteq \Delta$, we write $\Psi ⊢ ¬{\land}\Phi$ iff $\forall \phi \in ¬{\land}\Phi: \Psi ⊢ \phi$, or equivalently, iff $\exists \phi \in ¬{\land}\Phi \suchthat \Psi ⊢ \phi$, thus, whenever $\Psi$ permits to deduce the negation of at least one of the formulas in $\Phi$.
...
An undercut is maximally conservative when $\Phi' = \Phi$ in the above definition and $\forall \Psi' \subset \Psi: \Psi' ⊬ ¬{\land}(\Phi')$.
