...
Let $\Delta^\text{ordered}$ be a finite set of formulas is predicate logic with a deduction operator $⊢$. 

As we will see, negation of conjunctive formulas play a special role in the approach proposed by the authors. It will be convenient in what follows to be able to treat formulas such as $¬(a \land b)$ and $¬(b \land a)$ as the same mathematical object. To that effect, informally speaking, we define $\Delta$ as the set of formulas corresponding to $\Delta^\text{ordered}$ except that in $\Delta$, negation of conjunctive formulas are considered to be equal when they only differ on the ordering of their conjuncts inside the negation. We also define $\land(\Phi) \in \Delta$, given $\Phi \subseteq \Delta$, as the conjunction of the formulas in $\Phi$, where the ordering of the formulas in $\Phi$ “does not matter”.
The following remark defines $\Delta$ and $\land(\Phi)$ formally, and may be skipped without loss of continuity.
\begin{remark}
	To proceed formally, given $\Phi \subseteq \Delta^\text{ordered}$, let $\land(\Phi) \subseteq \Delta^\text{ordered}$ denote the set of conjunctions of the formulas in $\Phi$ following whatever ordering of the formulas in $\Phi$.
Define an equivalence relation $\equiv$ on $\Delta^\text{ordered}$ as follows: two formulas are equivalent, $\alpha \equiv \beta$, iff $\alpha = \beta \lor [\exists \Phi \subseteq \Delta^\text{ordered} \suchthat \alpha \in (¬{\land}(\Phi)) \land \beta \in (¬{\land}(\Phi))]$, thus, iff they are equal or they both negate the same conjunctive formula when ignoring the order of the sub-formulas in the conjunction. For example, given $a, b \in \Delta$, $¬(a \land b) \equiv ¬(b \land a)$.
Define $\Delta = \Delta^\text{ordered} / \equiv$ as the set of equivalence classes of $\equiv$, called “unordered formulas”.
We extend $⊢$ on the domain of unordered formulas: given $\Phi \subseteq \Delta, \psi \in \Delta$, $\Phi ⊢ \psi$ iff $\cup(\Phi) ⊢ \cup(\psi)$.
