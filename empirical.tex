...
\begin{example}[Model about ballistics, with symmetric attack]
	This disagreement can be modeled using the atoms $\mathit{lp}$ for \emph{The classical laws of physics are correct}, $\mathit{g}$ for \emph{There is a gigantic plot} and $\mathit{b}$ for $\emph{A bullet has been fired…}$.
	$\Delta = \set{\mathit{lp},\allowbreak \mathit{g}, \mathit{lp} → \mathit{b}, \mathit{g} → ¬\mathit{lp}}$.
	The undercut tree arguing for $\mathit{b}$ has root $\set{\mathit{lp},\allowbreak \mathit{lp} → \mathit{b}}$, having as child $\set{\mathit{g}, \mathit{g} → ¬\mathit{lp}}$.
\end{example}
This model seems to not adequately capture the featured disagreement: the asymmetric attack from $\set{\mathit{g}, \mathit{g} → ¬\mathit{lp}}$ against the conclusion of the expert suggests that its conclusion has been defeated and that the ballistic expert is wrong. On the contrary, it is very reasonable to assume that the very fact that the expert uses the classical laws of physics in her reasoning implicity entails that she considers that they are correct. 
\begin{example}[Model about ballistics, with asymmetric attack]
	This disagreement can be modeled using the atoms $\mathit{lp}$ for \emph{The classical laws of physics apply} and $\mathit{b}$ for $\emph{A bullet has been fired from the position … in the room}$.
...
\commentOCf{Dans cet exemple, il n’y a pas d’arbre pour flies(Tw) dont la racine soit undefeated, ce qui suggère que flies(Tw) ne peut pas être raisonnablement conclu à cause du contre-argument concernant les penguins (et c’est bien ce que cet exemple semble vouloir montrer intuitivement). Mais la conclusion semble changer radicalement si la base inclut aussi $¬¬fly(birds)$, par exemple. En effet, l’arbre ayant la même racine que l’exemple admet maintenant un troisième nœud, qui vient attaquer l’argument sur les penguins, argument avec support $\set{¬¬fly(birds), birds(pg) \land ¬fly(pg) → ¬fly(birds)}$ et avec conclusion $¬(birds(pg) \land ¬fly(pg))$. Aurais-je raté qqch. ?
%V1, defeated (desired intuitively):
%fb and bT ⇒ fT
%bp and ¬fp ⇒ ¬fb
%
%V2, we can warrant fT (undesired):
%fb and bT ⇒ fT
%bp and ¬fp ⇒ ¬fb
%¬¬fb
%
%We should mandate that a coherent claim be made, including everything that is required during the argument: fT and fb and ¬¬fb. Then everything is always defeated.
%
%Example where we add ¬¬stuff to get warranted conclusion. Problem is that some implications are implied in the discourse, and the modeler makes a hypothesis on those.
}
