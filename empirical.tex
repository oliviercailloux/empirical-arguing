...
Given some set $N$, we represent a tree using a pair $(r, u)$, where $r \in N$ is called the root of the tree, and  $u \subseteq N × N$ is called the successor relation. Define the predecessor relation as $u^{-1}$. 
We write $u[n]$ for the set of nodes reachable from $n$ following $u$, where the brackets designate the reflexive and transitive closure. We define the set of nodes as the set of elements $u[r] \subseteq N$ reachable from the root, and require $u$ to associate at most one predecessor to each element (thus $\forall n \in N: \card{u^{-1}(n)} ≤ 1$). 
