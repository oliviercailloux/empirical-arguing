...
\end{definition}
\begin{definition}
	Consider some finite $\Delta \subset \allformulas$. Define the implicit disagreements from $\Delta$ as $I_\Delta$ = $\set{¬\Psi \suchthat \phi \in \Delta, \Psi = \set{\psi_1, \psi_2, …} \subseteq \Delta \text{ and } (\Psi, ¬\phi) \text{ is an argument}}$. Here, $¬\Psi$ represents the formula $¬\psi_1 \lor ¬\psi_2 \lor …$.
	\commentOC{In the theorem, any set of implicit dis would suffice, where $¬\psi$ is anything that contradicts $\Psi$.}
\end{definition}
Note that $¬\Psi$ can arguably be considered as an implicit disagreement in the provided condition, as it is entailed by $\phi$, which is in $\Delta$.
\begin{definition}
	A model $\Delta$ has \emph{only trivial warrants} iff its set of warranted propositions is $\set{\phi \in \allformulas \suchthat \Delta \vdash \phi \land \Delta \nvdash ¬\phi}$.
\end{definition}

\commentOC{Strange. If Delta has any non-consensual proposition, then anything holds and Delta has only non-consensual propositions! Need to assume that there is an argument for $¬\phi$.}

\begin{conjecture}
	Consider a finite $\Delta \subseteq \allformulas$ and the corresponding implicit disagreements $I_\Delta$. If $\Delta$ is robust considering the implicit claims in $I_\Delta$, then $\Delta$ has only trivial warrants.
\end{conjecture}
\begin{proof}
	Consider $\phi \in \Delta$. Assume $\phi$ non consensual in $\Delta$. 
	...
	Given a node $\Psi$ in an undercut tree $u$, let $n(\Psi, u)$ denote $¬\Psi$ if $¬\Psi$ is not used in $u$, $¬¬(¬\Psi)$ if $¬\Psi$ is a node in $u$ but $¬¬(¬\Psi)$ isn’t, and so on. (Note that as $u$ is finite, there is necessarily some formula $(¬¬)^k(¬\Psi)$ that is not used in $u$).
