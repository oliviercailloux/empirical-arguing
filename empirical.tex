...
Given some set $N$, we represent a tree rooted at $n \in N$ using a pair $(n, s)$ where $s \subseteq N × N$ represents its successor relation (with $s$ connected and defining at most one path from the root to any $n' \in N$). 
