...
\begin{example}[Continuing \cref{ex:abstract}]
	\label{ex:abstUnder}
	\Cref{fig:utx} shows the sole undercut tree arguing for $x$ in our running example. There is only one such tree as the only argument that can be built for $x$ from $\Delta$ is $(\set{x}, x)$. Accordingly, it is the tree $(\set{x}, u_{\set{x}})$, or $u_{\set{x}}$ for short, with root $\Phi_1 = \set{x}$. The set of children $u_{\set{x}}(\set{x})$ of the root is the singleton constituted by the only canonical undercut of $\set{x}$, that is, $\Phi_2 = \set{y, y → ¬x}$. 
...
\begin{example}[Model for \cref{ex:ball}, with disagreement left implicit]
	\label{ex:ball2}
