...
Given some set $N$, we represent a tree using its successor relation $u \subseteq N × N$, with $u$ connected, $u$ defining a single element that has no predecessor (thus an element $n \in N \suchthat u^{-1}(n) = \emptyset$), called the root of the tree, and $u$ associating at most one predecessor to each element (thus $\forall n \in N: \card{u^{-1}(n)} ≤ 1$). 
