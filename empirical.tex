...
As usual, we call the union of the domain and codomain of $u$, $N' = u(N) \cup u^{-1}(N)$, the set of nodes, and require $u$ connected, $u$ defining a single node that has no predecessor (thus an element $n \in N' \suchthat u^{-1}(n) = \emptyset$), called the root of the tree, and $u$ associating at most one predecessor to each node (thus $\forall n \in N': \card{u^{-1}(n)} ≤ 1$). 
...
\commentOC{TODO define undercuts with the claims. Mandatory for the following to be an argumentation tree. $\Delta = \set{a, ¬a, ¬¬a, ¬¬¬a}$. Consider the tree rooted at $a$, with undercuts $¬a$ and $¬¬¬a$, both having $¬¬a$ as undercut.}
